...
useEffect(() => {
  ...
  const handleAddinfoDocument = async () => {
    if (newDocInfo.link && newDocInfo.title) {
      ...
      console.warn("Title and link are required to add a document");
      return null;
    }
  };
  ...
  const handleAddInfo = async () => {
    ...
    try {
      const response = await fetch(`${api_url}/medical-comments`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const handleUpdateInfo = async (customInfoId: string) => {
    try {
      ...
      if (!response.ok) {
        console.error("Server error:", result);
        throw new Error(result?.error?.message || "Failed to update custom info");
      }
  
      console.log("Updated info:", result);
      setShowupdateResidentModal(false);
      fetchCustomInfos(); // Refresh list
    } catch (error) {
      console.error("Error updating custom info:", error);
    }
  };
  
  const openUpdateDocumenttModal = (doc: { id: string; drive_url: string; title: string }) => {
    setNewDocInfo({
      link: doc.drive_url || "",
      title: doc.title || "",
    });
    ...
    try {
      ...
      if (customDocId) {
        try {
          ...
          console.log("Resident updated with new doc ID");
        } catch (updateError) {
          console.error("Error updating resident medical info:", updateError);
        }
      } else {
        console.error("customDocId is null — Add failed");
      }
    } catch (error) {
      console.error("Error adding and updating doc resident:", error);
    }
  };
  ...
  console.log("Medical Data:", document);


return (
  <div className="min-h-screen p-8 relative">
    <div className="w-[90%] mx-auto flex justify-between items-center mb-8 mt-8">
